    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(v2f i): SV_Target
            {
                ...
                float4 col = lerp(tex2D(_MainTex, uv), tex2D(_MainTex2, uv), _Interpolation);
                float intensity = col.a;
                col *= saturate(_Distance * (tex2D(_ColorSchemeTex, float2(intensity, uv.y)) * _Intensity * saturate(_Distance)) - 0.5);
